...
def calc_pressure_struct(input_file, pressure_dat_file, t):
    ...
    with open(input_file) as f:
        lines = f.readlines()
        for line in lines:
            match = re.search(r"<Number_of_time_steps>\s*(\d+)\s*</Number_of_time_steps>", line)
            if match:
                num_time_steps = int(match.group(1))
        
    with open(pressure_dat_file) as f:
        header = f.readline().strip().split()
        n_steps_file = int(header[0])
        _ = int(header[1])  # number of Fourier modes, not used here

        if n_steps_file != num_time_steps + 1:
            print(f"Warning: pressure.dat has {n_steps_file} steps but input file has {num_time_steps + 1}")

        # Load the rest of the file
        data = np.loadtxt(f)

    times = data[:, 0]
    pressure_full = data[:, 1]

    t = np.asarray(t, dtype=int)
    pressure = pressure_full[t]
